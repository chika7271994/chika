...
//データベース登録
if ($_POST['submit_add_plan']) {
	$sql = 'INSERT INTO plan (name, tell, mail, kind, time, message) VALUES(?, ?, ?, ?, ?, ?)';
	$statement = mysqli_prepare($database, $sql);
	mysqli_stmt_bind_param($statement, 'ssssss', $_POST['name'], $_POST['tell'], $_POST['mail'], $_POST['kind'], $_POST['time'], $_POST['message']);
...
echo "お名前　　　　 :　" .$_POST['name'];
echo nl2br("\n");
echo "お電話番号　　 :　" .$_POST['tell'];
echo nl2br("\n");
echo "メールアドレス :　" .$_POST['mail'];
echo nl2br("\n");
echo "ご希望プラン　 :　" .$_POST['kind'];
echo nl2br("\n");
echo "ご希望時間　　 :　" .$_POST['time'];
echo nl2br("\n");
echo "備考　　　　　 :　" .$_POST['message'];
echo nl2br("\n");
echo nl2br("\n");
echo "以上の内容でご予約致しました。";
echo nl2br("\n");
